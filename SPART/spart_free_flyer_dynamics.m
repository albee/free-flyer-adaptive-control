...
classdef spart_free_flyer_dynamics < handle
    properties
        ...
        R0 = [];  % Robot base orientation relative to inertial frame (DCM)
...
%             fnplt(self.pp,'r',2)
            ...
            scatter3(r_I(1,end),r_I(2,end),r_I(3,end),350,'black','filled');  % plot end effector
            ...
            quiver3(self.r0(1), self.r0(2), self.r0(3), self.control(1)/30.0, self.control(2)/30.0, 0,...
                'Color', 'black', 'LineWidth', 2.0, 'AutoScale', 'on');  % show control inputs on base
        end
        
        %{
        Plots a history of state vectors. Uses the 3D state convention!
        %}
        function fig = plot_from_history_FK3(self, fig, state_hist, control_hist)
            for i = 1:1:size(state_hist,1)
                ...
                % convert to SPART convention
                self.r0 = r;
                self.R0 = eul2rotm(q0(end:-1:1)');  % NOTE eul2rot convention is ZYX
                self.qm = [q1; q2];
